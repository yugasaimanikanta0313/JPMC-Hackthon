        ...
        if self.db is not None:
            ...
            if vector:
                try:
                    ...
                    if rows:return [(Chunk(project_id=project_id,document_id=str(x.get("document_id",x.get("_id"))),title=x.get("title","Project evidence"),content=x.get("content",""),verified=True,metadata={"source_type":x.get("source_type"),"repository":x.get("repository"),"commit_sha":x.get("commit_sha"),"path":x.get("path"),"language":x.get("language"),"access_scope":x.get("access_scope","project")}),float(x.get("vector_score",0))) for x in rows]
